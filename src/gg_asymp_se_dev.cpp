                ...
                y[3] = (c>>6)&mask2;
				        ...
				        bin_pointer=pointer_bins[bin_index];
                                ...
                                if(global_snp_index==(selected_snp_index-1) & read_sel_snp==false ){
                                       sel_snp_local_index=snp_index+1;
                                       cout << "################" << endl;
                                       cout << "current global_snp_index is: "<< global_snp_index << endl;
                                       for (int itmp=0;itmp<pointer_bins.size();itmp++)
                                        cout << "pointer_bins  is " << pointer_bins[itmp] << "size is: " <<  allgen_mail[pointer_bins[itmp]].index <<  endl;
                                       cout<<"Adjust bin_index is: "<< bin_index << endl;
                                       cout<<"Adjusted local index of target SNP (1-based): "<<sel_snp_local_index<<endl;
...
string tp=command_line_opts.Target_Path;
...
for (int j=0;j<Nbin;j++)
        if (selected_snp_index>0){
                // Boyang change here. added the condition checking selected_snp_index.  If selected snp is not in the list, then no need to exclude the target SNPs.
...
/////gxe computation
	   ...
	   if(remove_self_inter==true){
                if (selected_snp_index>0){ // Add condition checking if selected_snp_index is a valid number
	...
	//  for (int env_index=0;env_index<Nenv;env_index++){
        ...
        if (bin_index == gxgbin){
                ...
                // this for loop added Nov 21th, want to normalize the GxE column
                // for (int i = 0; i < env_all_zb.cols(); i++){
                //         double mean_sel_snp=env_all_zb.col(i).array().sum()/Nindv;
                //         env_all_zb.col(i).array() = env_all_zb.col(i).array() - mean_sel_snp;
                //         double sd_sel_snp = std::sqrt((env_all_zb.col(i).array() - env_all_zb.col(i).array().mean()).square().sum()/(env_all_zb.col(i).array().size()-1));
                //         env_all_zb.col(i).array() = env_all_zb.col(i).array()/sd_sel_snp;
                // }
...
for (int jack_index=0;jack_index<Njack;jack_index++){
	 ...
	 if(remove_self_inter==true) // self-interaction
                if (selected_snp_index>0){ // Boyang: check the validity of selected_snp_index
                        if(sel_snp_jack==jack_index&& annot_bool[selected_snp_index-1][bin_index]==1)
                                 exclude_sel_snp=true;
                }
